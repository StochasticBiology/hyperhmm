...
#' Bubble plot from fitted model
#'
#' @param fitted.obj (fitted model)
#' @param labels (vector of characters) optional set of labels for features in plot
#' @param formatted (Boolean, default FALSE) redundant, unused
#'
...
#' fitted = HyperHMM(m, nboot=0)
#' plot_bubbles(fitted)
plot_bubbles = function(fitted.obj,             # output data structure. either just a matrix of probabilities (formatted == F) or a dataframe output from the HyperHMM wrapper with means and sds (formatted == T)
  ...
  if(is.null(labels)) {
    g.1 = ggplot2::ggplot(bp.df, ggplot2::aes(x=order, y=feature)) +
      ggplot2::geom_point(ggplot2::aes(size=prob), colour="#CCCCCC") +
...
#'
#' @param fitted.obj (fitted model)
#' @param pfg.layout ("matrix", "tree", "other", default "matrix") how to lay out plot
#' @param curvature (double, default 1) curvature of edges
#'
#' @return A ggplot output
#' @export
#' @examples
...
#' fitted = HyperHMM(m, nboot=0)
#' plot_pfg(fitted)
plot_pfg = function(fitted.obj,              # list of transitions between states
...
#' Hypercube plot from fitted model
#'
#' redundant? older less flexible version -- will be sparsely commented
#'
#' @param fitted.obj (fitted model)
#' @param use.width (Boolean, default TRUE) use line width to display edge weights?
#' @param duplicate.offset (double, default 0) vertical offset for nodes in identical positions
#' @param lab.size (double, default 3) size for edge labels
#' @param p.size (double, default 1) point size
#' @param node.labels (Boolean, default TRUE) node labels, yes or no?
#' @param seg.labels (Boolean, default TRUE) line segment labels, yes or no?
#' @param threshold (double, default 0) ignore edges under a threshold in the hypercube plot
#' @param break.redundancy redundant, unused
#' @param rotate.phi (Boolean, default FALSE) rotate states out of the page (in case of trajectories bunched up near the top/bottom)
#'
#' @return A ggplot output
#' @export
#' @examples
#' m = matrix(c(0,0,1,0,1,1), byrow=TRUE, ncol=3)
#' fitted = HyperHMM(m, nboot=0)
#' plot_hypercube(fitted)
plot_hypercube = function(fitted.obj,               # including set of transitions
  ...
  if(use.width) {
    cube.plot = ggplot2::ggplot() +
      ggplot2::geom_segment(data=segments, ggplot2::aes(x=src.z, y=src.x, xend=dest.z, yend=dest.x, size=count/max(count)), color="#CCCCCC") +
      ggplot2::scale_size_identity() +
      ggplot2::geom_point(data = coords[safe.nodes == T,], ggplot2::aes(x=z, y=x), size=p.size, color="#444444") +
...
#' Panel plot for model output
#'
#' @param fitted.obj (fitted model) XXX
#' @param legacy redundant, unused
#' @param label (character) optional title for plot
#'
#' @return A ggplot output
#' @export
#' @examples
#' m = matrix(c(0,0,1,0,1,1), byrow=TRUE, ncol=3)
#' fitted = HyperHMM(m, nboot=0)
#' plot_standard(fitted)
plot_standard = function(fitted.obj, legacy=FALSE, label="") {
      plot_bubs = plot_bubbles(fitted.obj, formatted=TRUE) + ggplot2::ggtitle(label)
      plot_flux = plot_hypercube_flux(fitted.obj, thresh = 0.02) +
        ggplot2::theme(legend.position = "none")
      plot_diag = plot_pfg(fitted.obj, pfg.layout="matrix")
...
#'
#' @param fitted.obj (fitted model)
#' @param thresh (double, default 0.05) minimum flux threshold for edge plotting
#' @param node.labels (Boolean, default TRUE) node labels, yes or no?
#' @param use.probability (Boolean, default FALSE) use probability instead of flux?
#'
#' @return A ggplot output
#' @export
#' @examples
#' m = matrix(c(0,0,1,0,1,1), byrow=TRUE, ncol=3)
#' fitted = HyperHMM(m, nboot=0)
#' plot_hypercube_flux(fitted)
plot_hypercube_flux = function(fitted.obj, thresh = 0.05, node.labels = TRUE, use.probability = FALSE) {
  my.post = fitted.obj
  ...
  if(use.probability == TRUE) {
    ...
    this.plot = ggraph::ggraph(trans.g, layout="sugiyama", layers=layers) +
      ggraph::geom_edge_link(ggplot2::aes(edge_width=Flux, edge_alpha=Flux)) +
      ggraph::scale_edge_width(limits=c(0,NA)) +
      ggraph::scale_edge_alpha(limits=c(0,0.5)) +
      ggplot2::theme_void()
  }

...
void import_data(string file_name, vector<string>& data, int *len){
  ...
  if(!in.is_open())
  {
    Rcpp::Rcout << "Can't find input file: " << file_name << "!\n";
    myexit(0);
  }
  std::string str;
...
List run_inference(vector<string>& data, int L, int n_boot, string name, double& time, int rw_boot) {
  ...
  Rcpp::Rcout << "Mean ordering matrix from sampling:\n";
  Rcpp::Rcout << rw << endl;
  mean.slice(0) = rw;
  sd.slice(0) = rw;
  ...
  Rcpp::Rcout << "Running bootstrap resamples\n";
  int outboot;
  for(int bootref=0; bootref<n_boot; bootref++){
    ...
    if(outboot == 1)
    {
      Rcpp::Rcout << "Bootstrap number: " << bootref+1 << "\n";
    }
...
int main(int argc, char** argv){
  ...
  if(oldform == 1)
  {
    ...
    Rprintf("It looks like you've used the old command-line argument specification, which is preserved for backwards compatibility. The newer format may allow more flexibility:\n");
    help();
    n_boot = atoi(argv[3]);
    rw_boot = atoi(argv[6]);
    cross_sectional = atoi(argv[5]);
    snprintf(obsfile, sizeof(obsfile), "%s", argv[1]);
    snprintf(labelstr, sizeof(labelstr), "%s", argv[4]);
  }
  else
  {
    ...
    for(i = 1; i < argc; i+=2)
    {
      ...
      else { Rprintf("Didn't understand argument %s\n", argv[i]); i--; }
    }
    if(filelabel == 0)
    {
      snprintf(labelstr, sizeof(labelstr), "%s-out", obsfile);
    }
  }
  ...
  if(strcmp(obsfile, "") == 0)
  {
    Rprintf("I need at least an input file!\n\n");
    help();
    myexit(0);
  }
  ...
  if(cross_sectional == 1){
    ...
    Rcpp::Rcout << "From " << obsfile << " I read " << data.size() << " entries and am assuming " << L << " features\n\n";
              ...
              NumericVector outputinput = 0)
{
  ...
  if(initialstates.isUsable())
  {
    ...
    for(int i = 0; i < obs.nrow(); i++)
    {
      ...
      for(int j = 0; j < L; j++)
      {
        snprintf(tmps, sizeof(tmps), "%i", (int)_initialstates(i,j));
        str += tmps[0];
      }
      str += ' ';
      for(int j = 0; j < L; j++)
      {
        snprintf(tmps, sizeof(tmps), "%i", (int)obs(i,j));
        str += tmps[0];
      }
      ...
      for(int j = 0; j < L; j++)
      {
        ...
        {
          Rcpp::Rcout << "Record " << i << " has a reversible transition\n";
          myexit(0);
        }
      ...
      if(sum_init == sum_obs)
      {
        Rcpp::Rcout << "Record " << i << " has identical initial and end states\n";
        myexit(0);
      }
